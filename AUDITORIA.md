...
| **S1b — qs/express + NestJS 10**         | ⏸️ **Adiado (justificado)** | `express@4.22.1` fixa `qs` em `~6.14.0` e **não existe qs seguro nesse range**; o fix real exige subir `@nestjs/platform-express` → **NestJS 11 (major)**. Override foi tentado e **revertido** (conflita com o range; ficaria no-op enganoso). Idem highs transitivos `multer`/`js-yaml`/`lodash`. | Diagnóstico registrado; **não** forçado                                                   |
...
**Recomendação para S1b (fora da Fase B):** planejar a migração **NestJS 10 → 11**
como tarefa própria, com branch dedicada, `npm audit` limpo como critério e a
suíte e2e como rede de segurança. Não fazer por impulso — é mudança de framework.
